...
argument-hint: "--source <源码目录> [--output <输出目录>] [--module <模块路径>] [--package <包名>] [--init-only] [--refresh-entries] [--doc-quality-threshold <0-1>]"
user-invocable: true
...
for each entry:
  ...
  去重后将每个文件的 path、language、Codegraph CLI `files -j` 返回的 nodeCount
  写入 <codebook_root>evidence/<entry-id>/inventory.json
```
